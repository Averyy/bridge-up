...
class TestNetworkBackoff(unittest.TestCase):
    ...
    def test_network_failure_triggers_backoff(self, mock_update, mock_scrape):
        ...
        result = process_single_region((test_key, test_info))
        self.assertEqual(result, (False, 0))

        # Check backoff was set
        with region_failures_lock:
            self.assertIn(test_key, region_failures)
            failure_count, next_retry = region_failures[test_key]
    ...
    def test_recovery_after_network_failure(self, mock_update, mock_scrape):
        ...
        # First fail
        mock_scrape.return_value = []
        process_single_region((test_key, test_info))
        ...
        with region_failures_lock:
            failure_count, _ = region_failures[test_key]
            region_failures[test_key] = (failure_count, datetime.now())  # Set retry time to now
        ...
        with region_failures_lock:
            self.assertNotIn(test_key, region_failures)
